...
from pure_rnn_model import my_rnn
import torch.nn as nn


# 函数输入：文件名
# 函数输出：二维矩阵，行数为样本个数
#          共三列，第一列为相似度，第二列为切词成的单词组成的列表，第二列为切词后的单词组成的列表
...
if __name__=='__main__':
    ...
    drop_out_prob=0
    # 初始化模型
    rnn = my_rnn(embed_size,batch_size,hidden_size,num_layers,drop_out_prob)
    rnn=rnn.to(device)
    # -----------以上是模型参数部分-----------------------
    ...
    learning_rate = 0.1
    # 优化器（梯度更新）
    optimizer = torch.optim.Adadelta(filter(lambda x: x.requires_grad, rnn.parameters()), lr=learning_rate)
    # 损失函数
    ...
    while epoch < n_epoch:
        ...
        for idx, data in enumerate(train_data):
            ...
            optimizer.zero_grad()
            # 输出
            output = rnn(sentence_tensor1, sentence_tensor2)
        ...
        if np.mean(valid_loss) < best_loss:
            ...
            torch.save(rnn.state_dict(), "rnn_params.pkl")
